...
export const auditLogRepo = repoFactory(AuditLogEntity)
...
export const auditLogService = (log: FastifyBaseLogger) => ({
    ...
    async list({ platformId, cursorRequest, limit, userId, action, projectId, createdBefore, createdAfter }: ListParams): Promise<SeekPage<ApplicationEvent>> {
        ...
        if (createdAfter) {
            queryBuilder.andWhere('audit_log.created >= :createdAfter', {
                createdAfter,
            })
        }
        if (createdBefore) {
            queryBuilder.andWhere('audit_log.created <= :createdBefore', {
                createdBefore,
            })
        }
