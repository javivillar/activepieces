...
import { ActivepiecesError, ApEnvironment, ErrorCode, RefresquitoConfigureRepoRequest, RefresquitoGitRepo } from '@activepieces/shared'
...
export const refresquitoGitHelper = {
    commitAndPush,
    createGitRepoAndReturnPaths,
    validateConnection,
}

async function commitAndPush(git: SimpleGit, gitRepo: RefresquitoGitRepo, commitMessage: string): Promise<void> {
    await git.add('.')
...
async function createGitRepoAndReturnPaths(
    ...
    userId: string,
): Promise<{ flowFolderPath: string, git: SimpleGit, tablesFolderPath: string }> {
    assertSafeSlug(gitRepo.slug)
    const tmpFolder = path.join('/', 'tmp', 'refresquito-repo', gitRepo.projectId)
    try {
        await fs.rmdir(tmpFolder, { recursive: true })
    }
    catch {
        // ignore -- first run, nothing to clean up yet
    }
    ...
    const keyPath = path.resolve(path.join('tmp', 'refresquito-keys', gitRepo.id))
    ...
    await git.addConfig('user.name', `${identity.firstName} ${identity.lastName}`)
    return { git, flowFolderPath, tablesFolderPath }
}
...
function assertSafeSlug(slug: string): void {
    if (!SAFE_SLUG_PATTERN.test(slug) || slug === '.' || slug === '..') {
        throw new ActivepiecesError({
            ...
            params: { message: `invalid gitRepo.slug "${slug}"` },
        })
    }
}
...
function assertSafeKeyPath(keyPath: string): void {
    if (!SAFE_KEY_PATH_PATTERN.test(keyPath)) {
        throw new ActivepiecesError({
            code: ErrorCode.VALIDATION,
            params: { message: 'invalid ssh key path' },
        })
    }
}

async function validateConnection(request: RefresquitoConfigureRepoRequest): Promise<void> {
    ...
    const tmpFolder = path.join('/', 'tmp', 'refresquito-repo', nanoid(), 'validate')
    const keyPath = path.resolve(path.join('tmp', 'refresquito-keys', nanoid()))
    try {
        await fs.mkdir(tmpFolder, { recursive: true })
