...
export const appConnectionWorkerController: FastifyPluginAsyncZod = async (app) => {
    ...
    app.get('/:externalId', GetAppConnectionRequest, async (request): Promise<AppConnection> => {
        ...
        return {
            ...appConnection,
            value: await refresquitoSecretManagerService(request.log).resolveObject({ value: appConnection.value, projectIds: [enginePrincipal.projectId], platformId: enginePrincipal.platform.id, throwOnFailure: false }),
        }
    },
    )
